...
/*
  This library uses Timer2 to drive up to 8 servos using interrupts so no refresh activity is
required from within the sketch. The usage and method naming is similar to the Arduino software
servo library http://www.arduino.cc/playground/ComponentLib/Servo except that pulse widths can be in
microseconds or degrees. write() treats parameters of 180 or less as degrees, otherwise values are
milliseconds.


  A servo is activated by creating an instance of the Servo class passing the desired pin to the
attach() method. The servo is pulsed in the background to the value most recently written using the
write() method
...
class ServoTimer2
{
   public:
    ...
    uint8_t attach(int);  // attach the given pin to the next free channel, sets pinMode, returns
                          // channel number or 0 if failure the attached servo is pulsed with the
    ...
    void detach();
    void write(int);     // store the pulse width in microseconds (between MIN_PULSE_WIDTH and
...
class ServoArrayT2
{
   public:
    ...
    uint8_t attach(int);    // attach the given pin to the next free channel, sets pinMode, returns
                            // channel number or 0 if failure channels are assigned consecutively
                            // starting from 1 the attached servo is pulsed with the current pulse
                            // width value, (see the write method)
    void detach(int);       // detach the servo on the given channel
    void write(int, int);   // store the pulse width in microseconds (between MIN_PULSE_WIDTH and
