...
class MQTTService(BaseService):
    ...
    def __init__(self, broker: str, port: int, event_bus: EventBus, qos: QOSLevel, publish_interval: float = 5.0):
        """
        ...
        # Chache last data received from bus topics
    ...
    def configure_messaging(self, incoming: Optional[Dict[str, str]] = None, outgoing: Optional[Dict[str, str]] = None):
        ...
        :param incoming: Map of { "mqtt/topic": "internal.bus.topic" }
                        MQTT receive from broker → publish on bus
        :param outgoing: Map of { "internal.bus.topic": "external/mqtt/topic" }
                        Bus receive events → publish on MQTT broker
        """
        if incoming:
        ...
        if outgoing:
            ...
            for bus_topic in self._outgoing_map.keys():
                # Create dedicated callback for each topic
    ...
    async def _periodic_publish(self):
        """Periodically publish cached data to MQTT."""
        try:
    ...
    def _make_outgoing_handler(self, bus_topic: str):
        """Create a handler to publish bus events to MQTT."""
        mqtt_topic = self._outgoing_map[bus_topic]
        
        def handler(**kwargs):
            try:
                # Save to cache for periodic publishing
